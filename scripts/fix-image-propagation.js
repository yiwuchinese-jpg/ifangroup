...
const TEMP_EXTRACT_DIR = '/tmp/ifangroup-excel-extract-cascade';

async function run() {
    console.log('🚀 Starting Cascading Image Fill...');

    // Fetch all products, including their brand name to avoid collisions
    ...
    for (const file of excelFiles) {
        ...
        for (const [sRow, spuName] of sortedRowSPUs) {
            ...
            if (!targetProd.hasImage) {
                ...
                // Cascading logic: find the closest image on or above this row (+2 tolerance in case image is nudged down)
                ...
                if (matchedImageName) {
                    ...
                    if (fs.existsSync(imagePath)) {
                        try {
                            ...
                            console.log(`   📸 [Cascaded] ${parentFolder} -> ${spuName} (from image at row ${closestAnchor.row})`);
                        } catch (imgErr) { }
                    }
                }
            }
    ...
    console.log(`\n✅ Finished Execution! Successfully cascaded ${imageUploads} missing images.`);
}
