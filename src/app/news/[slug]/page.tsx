...
const components = {
    ...
    block: {
        h2: ({ children, value }: any) => {
            const id = value?.children?.map((child: any) => child.text).join('')
        ...
        normal: ({ children }: any) => <p className="text-lg md:text-xl text-slate-600 leading-relaxed font-medium mb-8">{children}</p>,
    ...
    marks: {
        strong: ({ children }: any) => <strong className="font-black text-slate-900 bg-slate-100 px-1">{children}</strong>,
    ...
    list: {
        bullet: ({ children }: any) => <ul className="list-none space-y-4 mb-12 text-lg md:text-xl text-slate-600 font-medium">{children}</ul>,
        number: ({ children }: any) => <ol className="list-decimal pl-6 space-y-4 mb-12 text-lg md:text-xl text-slate-600 font-medium font-mono">{children}</ol>,
    },
    listItem: {
        bullet: ({ children }: any) => (
...
export default async function NewsArticlePage({ params }: { params: Promise<{ slug: string }> }) {
    const resolvedParams = await params;
    const article = await client.fetch(articleBySlugQuery, { slug: resolvedParams.slug });
    const relatedArticles = await client.fetch(relatedArticlesQuery, { slug: resolvedParams.slug });
