...
export default function BrandSolarSystem({ brands }: { brands: Brand[] }) {
 ...
 {/* Clicked Brand Info Overlay (Glassmorphism Modal) */}
 <AnimatePresence>
 {activeBrand && (
 <motion.div
 ...
 className="absolute top-auto bottom-24 md:bottom-8 left-4 right-4 md:top-24 md:left-auto md:right-8 w-auto md:w-96 max-h-[60vh] md:max-h-none overflow-y-auto bg-white/80 backdrop-blur-2xl border border-slate-200 p-6 md:p-8 z-20 flex flex-col pointer-events-auto shadow-[0_20px_50px_rgba(0,0,0,0.1)]"
 >
 <button
 onClick={() => setActiveBrand(null)}
 className="absolute top-6 right-6 text-slate-400 hover:text-slate-900 transition-colors bg-slate-100 p-2"
 >
 <X className="w-5 h-5"/>
 </button>

 <div className="flex-grow flex flex-col">
 <span className="inline-block bg-brand-50 text-brand-700 font-bold tracking-[0.3em] uppercase text-[10px] px-3 py-1.5 mb-6 w-max">
 {activeBrand.series ? (SERIES_TRANSLATIONS[activeBrand.series] || activeBrand.series.toUpperCase()) :""}
 </span>

 {activeBrand.logo?.asset?.url ? (
 <div className="h-24 bg-slate-50 mb-6 p-4 flex items-center justify-center border border-slate-100">
 <img
 src={activeBrand.logo.asset.url}
 ...
 {activeBrand.description ||"Premium fluid control engineering systems designed for global infrastructure deployment. Discover the core capabilities and product matrix associated with this entity."}
 </p>
 </div>
 </div>
 ...
 className="mt-8 w-full bg-brand-600 hover:bg-brand-500 text-white font-bold uppercase tracking-widest text-xs py-4 flex items-center justify-center gap-2 transition-all group shadow-[0_0_20px_rgba(22,163,74,0.4)] hover:shadow-[0_0_30px_rgba(22,163,74,0.6)]"
 >
 Enter Brand Matrix
